...
impl Row {
    ...
    pub fn serialize(&self, schema: &Table) -> Result<Vec<u8>> {
        ...
        let mut serialized_var_offsets:Vec<u8> = Vec::new();

        for offset in variable_offsets.iter() {
            let low_byte: u8 = (offset & 0xFF) as u8; // Get the low byte
            let high_byte: u8 = (offset >> 8) as u8; // Get the high byte
            serialized_var_offsets.push(high_byte);
            serialized_var_offsets.push(low_byte);
        }
